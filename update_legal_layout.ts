...
const newComponent = `import React, { ReactNode } from 'react';
...
export const LegalPageLayout: React.FC<LegalPageLayoutProps> = ({ title, lastUpdated, intro, children, noticeText }) => {
  ...
  return (
    <div className="bg-[var(--background-color)] min-h-screen">
      ...
      {/* IMPORTANT NOTICE */}
      <Section className="py-8 bg-[#0A0C10] border-b border-[var(--border)]">
        <Container size="md">
...
fs.writeFileSync('src/components/legal/LegalPageLayout.tsx', newComponent);
